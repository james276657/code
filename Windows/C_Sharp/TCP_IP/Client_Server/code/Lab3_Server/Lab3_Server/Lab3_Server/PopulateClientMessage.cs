...
namespace CST415_Lab3_Server
{
    class PopulateClientMessage
    {
        ...
        public void Rvc_Message_Timestamp(rcmessage rcmsg, DateTime dt)
        {
            TimeSpan et;
            DateTime now = DateTime.Now;
            int elapsedTime;
            et = now - dt;
            elapsedTime = (int)et.TotalMilliseconds;
            rcmsg.rcvTimeStamp = elapsedTime.ToString();
        }
        public void Fill_Message_StandIn_Timestamp(ref byte[] bytes, int val)
        {
            string asciielapsedTime = val.ToString();
